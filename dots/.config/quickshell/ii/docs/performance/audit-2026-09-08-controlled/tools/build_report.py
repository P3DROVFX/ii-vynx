...
'cheatsheet_timetable_grid_only':'Calendário mensal — sem EventSidebar/pickers',
'notes_list_only':'Notas — apenas lista (sem NotesDetail)',
'family_nav_keep':'Navegação sequencial — cache ligado',
'family_nav_unload':'Navegação sequencial — cache desligado',
...
if data.get('cheatsheet_keybinds_keep',{}).get('case',{}).get('samePrewarm'):
 LABELS['cheatsheet_keybinds_keep']='Atalhos — cache ligado, mesma aba'
manifest={'createdUtc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'cases':{},'units':'MiB; CPU 100%=one logical CPU; GPU SM percent NVML', 'snapshot':json.loads((BASE/'preparation.json').read_text()).get('sourceHead') if (BASE/'preparation.json').exists() else '842411d065ada2b95a714dbeacc1387ada258aac'}
records=[]
...
for name,d in data.items():
 p={x['phase']:x for x in d['summary']}
 if 'gc' not in p or 'unloaded' not in p:continue
